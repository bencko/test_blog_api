...
class FeedView(generics.GenericAPIView, mixins.ListModelMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        Return all your posts in feed\n
            - only for authenticated users\n
            - have optional parameter "filter" = "readed"\
             or "unreaded(default)"\n
            EXAMPLE: curl http://127.0.0.1:8000/api/blog/feed?filter=unreaded\
             - return all unreaded posts in your feed
            EXAMPLE: curl http://127.0.0.1:8000/api/blog/feed?filter=readed\
             - return all readed posts in your feed
    ...
    def _filter_qs(self, request, queryset):
        ...
        for item in queryset:
            ...
            try:
                ...
                filtered_unreaded_qs = list(chain(
                    filtered_unreaded_qs,
                    qs_item)
                )
...
class UserPostsView(generics.GenericAPIView, mixins.ListModelMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        Return all posts of user\n
            - Allow any user\n
            EXAMPLE: curl http://127.0.0.1:8000/api/blog/12/posts/\
             - return all user posts which id=12\n
...
class SubscribeCreateOrListView(mixins.ListModelMixin,
    ...
    def get_queryset(self):
        return Subscribe.objects.filter(
            subscribeslist__owner=self.request.user
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        Return all your subscribes\n
            - only for authenticated users\n
            EXAMPLE: curl http://127.0.0.1:8000/api/blog/subscribes/\
             - return all your subscribes\n
        """
        return self.list(request, *args, **kwargs)
    ...
    def post(self, request, *args, **kwargs):
        ...
        Subscribe you on user\n
            - only for authenticated users\n
            EXAMPLE: curl --data "to=14" \
            http://127.0.0.1:8000/api/blog/subscribes/\
             - subscribe you on user which id=14\n
        """
        return self.create(request, *args, **kwargs)
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            try:
                ...
                .get(to_id=request.data.get('to')):
                    subscribed = True
                    return Response(
                        {'detail': 'You already subscribe to this user'},
...
class SubscribeOperateView(generics.GenericAPIView, mixins.DestroyModelMixin):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        Unsubscribe on user\n
            ...
            http://127.0.0.1:8000/api/blog/subscribes/2/\
             - delete your subscribe which id=2
        """
...
class ReadedPostsView(generics.GenericAPIView, mixins.CreateModelMixin):
    ...
    def post(self, request, *args, **kwargs):
        ...
        Mark post as readed\n
            ...
            EXAMPLE: curl  -H 'Content-Type: application/json'\n
            --data '{"post":"14"}' \
            http://127.0.0.1:8000/api/blog/readed/\
             - mark post which id=14 as readed\n
        """
        ...
        if not already:
            ...
            if in_feed:
                return self.create(request, *args, **kwargs)
            return Response(
                {'detail': 'This post is not in feed'},
                status=status.HTTP_400_BAD_REQUEST
            )
        return Response(
            {'detail': 'Already marked "as read"'},
            status=status.HTTP_200_OK
        )
